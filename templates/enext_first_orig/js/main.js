(function(window) {
	...
	BX.addCustomEvent('onAjaxSuccess', function() {
		var picLazyLoad = document.body.querySelectorAll('.lazy-load');
		if(!!picLazyLoad) {
			for(var i in picLazyLoad) {
				if(picLazyLoad.hasOwnProperty(i) && BX.type.isDomNode(picLazyLoad[i])) {
					if(picLazyLoad[i].getAttribute('data-src')) {
						BX.style(picLazyLoad[i], 'background-image', 'url(' + picLazyLoad[i].getAttribute('data-src') + ')');
						picLazyLoad[i].removeAttribute('data-src');
						BX.removeClass(picLazyLoad[i], 'lazy-load');
					}
				}
			}
		}
