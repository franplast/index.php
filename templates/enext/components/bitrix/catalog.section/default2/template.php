...
$generalParams = array(
	...
	'MESS_BTN_DELAY' => $arParams['MESS_BTN_DELAY'],
...
$containerName = 'container-'.$navParams['NavNum'];?>
	...
	<?if(!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS'])) {
		...
		<?foreach($arResult['ITEM_ROWS'] as $rowData) {			
			...
			if($arParams["WIDESCREEN_MODE"] != "Y") {?>
				<div class="row product-item-row" data-entity="items-row">
			<?}
			foreach($rowItems as $item) {?>
				<div class="<?=($arParams['WIDESCREEN_MODE'] != 'Y' ? 'col-xs-12 '.($rowData['VARIANT'] == 2 ? 'col-md-4' : 'col-md-3') : 'product-item-col')?>"<?=($arParams["WIDESCREEN_MODE"] == "Y" ? "data-entity='item-col'" : "")?>>
...
if($arParams['HIDE_SECTION_DESCRIPTION'] !== 'Y' && !empty($arResult['DESCRIPTION'])) {
	if($navParams['NavPageNomer'] <= 1) {?>
		<div class="catalog-section-desc"><?=$arResult['DESCRIPTION']?></div>
	<?}
}
...
<script type="text/javascript">
	...
	var <?=$obName?> = new JCCatalogSectionComponent({
		...
		wideScreenMode: !!'<?=($arParams["WIDESCREEN_MODE"] === "Y")?>',
		imgLazyLoad: !!'<?=($arParams["IMG_LAZYLOAD"] === "Y")?>',
