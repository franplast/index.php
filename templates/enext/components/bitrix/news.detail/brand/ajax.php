...
if($request->isAjaxRequest() && $request->get("action") == "changeSectionLink") {
    ...
    {
        $rsElements = CIBlockElement::GetList(array(), array("ACTIVE" => "Y", "IBLOCK_ID" => $arIblock["ID"], "SECTION_GLOBAL_ACTIVE" => "Y", "PROPERTY_BRAND" => $brandId), false, false, array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID"));
        while ($arElement = $rsElements->GetNext())
        {
            $GLOBALS["arBrandsProdFilter"]["ID"][] = $arElement["ID"];
        }

        $GLOBALS["arBrandsProdFilter"]["IBLOCK_ID"][] = $arIblock["ID"];
    }
