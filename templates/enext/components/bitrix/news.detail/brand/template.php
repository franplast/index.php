...
/**************
*	changes in the file are marked with a comment "// Modificate"
*		
*************/
		...
		if(!empty($arResult["PRODUCTS_IDS"])) {?>			
			<div class="brands-detail-products-container" data-entity="tab-container" data-value="products">
				...
				<div class="brands-detail-sections-links" data-entity="links">
					<div class="brands-detail-section-link active" data-brand-id="<?=$arResult["ID"]?>"  data-section-id="0"><?=Loc::getMessage("BRANDS_ITEM_DETAIL_PRODUCTS_SECTIONS_ALL")?><span><?=count($arResult["PRODUCTS_IDS"])?></span></div>
					<?if(!empty($arResult["SECTIONS"])) {
						foreach($arResult["SECTIONS"] as $arSection) {?>
							<div class="brands-detail-section-link" data-brand-id="<?=$arResult["ID"]?>" data-section-id="<?=$arSection['ID']?>"><?=$arSection["NAME"]?><span><?=$arSection["COUNT"]?></span></div>
						<?}
						unset($arSection);
					}?>
				</div>
				<div class="brands-detail-products">
					<?//PRODUCTS//
					$GLOBALS["arBrandsProdFilter"] = array("ID" => $arResult["PRODUCTS_IDS"], "IBLOCK_ID" => $arResult["IBLOCKS_IDS"]); // Modificate
					$APPLICATION->IncludeComponent("zs:catalog.section", ".default", // Modificate
						array(
...
$arJSParams = array(
	...
	"ITEM" => array(
		// "PRODUCTS_IDS" => $signedProductsIds // Modificate
		"PRODUCT_IDS" => $arResult["PRODUCTS_IDS"] // Modificate
	),
	"VISUAL" => array(
		"ID" => $mainId
	)
