	...
	<?foreach($arResult["ITEMS"] as $arItem) {
		...
		<div class="contacts-item">
			...
			<div class="contacts-item-map">
				<?$mapData = array();
				if(!empty($arItem["MAP"])) {
					$arTmp = explode(",", $arItem["MAP"]["VALUE"]);
				...
				if(count($mapData["PLACEMARKS"]) == 1) {
					...
					$mapData["google_scale"] = "15";
				}?>
				<?$APPLICATION->IncludeComponent("bitrix:map.google.view", "",
					array(
						"API_KEY" => !empty($arItem["MAP"]["API_KEY"]) ? $arItem["MAP"]["API_KEY"] : Bitrix\Main\Config\Option::get("fileman", "google_map_api_key"),
						"CONTROLS" => array(
