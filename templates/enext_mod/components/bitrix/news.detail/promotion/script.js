(function() {
	...
	window.JCNewsDetailPromo = function(arParams) {
		...
		if(typeof arParams === 'object') {
			...
			this.item.showTimer = arParams.ITEM.SHOW_TIMER;
			this.item.productIds = arParams.ITEM.PRODUCT_IDS;
	...
	window.JCNewsDetailPromo.prototype = {
		init: function() {
			...
			if(this.errorCode === 0) {
				...
				this.obSectionsLinks = this.obItem.querySelector('.promotions-detail-sections-links');
		...
		initSectionsLinks: function() {
			var sectionLinks = this.obSectionsLinks.querySelectorAll('.promotions-detail-section-link'),
				haveActive = false;
		...
		changeSectionLink: function(event) {
			...
			if(!BX.hasClass(BX.proxy_context, 'active') && sectionId) {
				...
				if(!!itemProductsContainer) {					
					...
					BX.ajax.post(
						BX.message('PROMOTION_TEMPLATE_PATH') + '/ajax.php',
						{
							action: 'changeSectionLink',
							REQUEST_URI: window.location.href,
							siteId: BX.message('SITE_ID'),
							parameters: this.config,
							productIds: this.item.productIds,
							sectionId: sectionId
						},
