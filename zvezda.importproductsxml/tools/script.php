...
class importProductsXml
{
    ...
    public $arCategories = []; // массив категорий из xml, заполняется методом getArrayCategoriesXml
    ...
    public $HlBlockIdCategories; // Справочник категорий к ИБ
    public $HlBlockIdParams; // Справочник
    ...
    public $arFieldsSku = [
        ...
        "picture" => [ // Это по идее можно удалить
            "NAME" => "Детальная картинка",
            "CODE" => "DETAIL_PICTURE"
        ]
    ];
    ...
    {
        $this->shopIblockId = 13; // ToDo вынести в настройки модуля в БД
        $this->productIblockType = "catalog"; // ToDo вынести в настройки модуля в БД
        $this->iterationStep = 50; // ToDo вынести в настройки модуля в БД
        $this->mapingIblockId = 266; // ToDo вынести в настройки модуля в БД
        ...
        $this->HlBlockIdCategories = 4; // ToDo вынести в настройки модуля в БД. запретить изменение пользователем или предупредить Или вынести в настройки на отдельную вкладку, типа "системные" Или научить модуль самостоятельно создавать под себя такие справочники
        $this->HlBlockIdParams = 3;  // ToDo вынести в настройки модуля в БД. запретить изменение пользователем или предупредить Или вынести в настройки на отдельную вкладку, типа "системные" Или научить модуль самостоятельно создавать под себя такие справочники
        ...
        if(!empty($shopId))
        {
            ...
            if(!$this->setXmlContentByShopId($this->shopId))
                exit();
            if(!$this->setShopNameContentByShopId($this->shopId))
                exit();
        }
        ...
        {
            // ToDo Если не переданы ни корректный id магазина, ни ссылка на файл отработать исключение.
    ...
    public function setXmlContentByShopId($shopId) // устанавливаем содержимое файла в переменную $this->xmlContent
    {
        ...
        if($arElement = $rsElement->Fetch())
        {
            ...
            {
                $this->xmlFileStatus = "<span style='color: forestgreen'>Файл доступен</span>";
                return true;
            }
            else
            {
                $this->xmlFileStatus = "<span style='color: #ff2a0c'>Файл не доступен</span>";
            }
        }
    ...
    public function setShopNameContentByShopId($shopId) // устанавливаем содержимое файла в переменную $this->xmlContent
    {
        global $el;
        $rsElement = $el->getList([], ["IBLOCK_ID" => $this->shopIblockId, "ID" => $shopId, "ACTIVE" => "Y", "!PROPERTY_FILE_REFERENCE" => false], false, false, ["IBLOCK_ID", "NAME"]);
        if($arElement = $rsElement->Fetch())
        {
            $this->shopName = $arElement["NAME"];
            return true;
        }
        return false;
    }
            ...
            for($i=$numOffer; $i<count($arOffers); $i++)
            {
                ...
                */
                ...
                if($productIblockId) // если у категории товара из xml проставлен ID ИБ
                {
                    ...
                    // Мы ищем $arOffers[$i]["prop"]] без name потому что св-ва передаются в param, а не в prop
                    ...
                    $xmlId = $this->xmlUrl."_".$arOffers[$i]["@attributes"]["id"]; // внешний код XML_ID
                    ...
                    $arProperties = [
                        ["CODE" => "MODEL", "NAME" => "Модель"],
                        ["CODE" => "SECTION_YML", "NAME" => "Структура разделов из YML"],
                        ["CODE" => "WEIGHT", "NAME" => "Вес"],
                        ["CODE" => "ARTICLE", "NAME" => "Артикул"],
                        ["CODE" => "OLD_PRICE", "NAME" => "Старая цена"],
                        ["CODE" => "BARCODE", "NAME" => "Штрих-код"],
                        ["CODE" => "ITERATION", "NAME" => "Дата итерации",],
                        ["CODE" => "SCRIPT_CREATE", "NAME" => "Создано скриптом", "PROPERTY_TYPE" => "L"],
                    ];

                    foreach($arProperties as $propertyCode => $property)
                    {
                        if(!$this->isExistPropertyIblock($property['CODE'], $productIblockId))
                        {
                            $PROPERTY_TYPE = $property['PROPERTY_TYPE'] ?: 'S' ;
                            $arFields = Array(
                                "NAME" => $property['NAME'],
                                "ACTIVE" => "Y",
                                "SORT" => "10010",
                    ...
                    if($skuIblockId) // если получен ИБ ТП
                    {
                        ...
                        $arProperties = [
                            ["CODE" => "ITERATION", "NAME" => "Дата итерации"],
                            ["CODE" => "SCRIPT_CREATE", "NAME" => "Создано скриптом", "PROPERTY_TYPE" => "L"],
                            ["CODE" => "URL", "NAME" => "Ссылка на источник"],
                            ["CODE" => "IN_STOCK", "NAME" => "В наличии", "PROPERTY_TYPE" => "L"]
                        ];
                        ...
                        foreach($arProperties as $propertyCode => $propertyValue)
                        {
                            ...
                            if(!$this->isExistPropertyIblock($property['CODE'], $skuIblockId))
                            {
                                $PROPERTY_TYPE = $property['PROPERTY_TYPE'] ?: 'S' ;
                                $arFields = Array(
                                    "NAME" => $property['NAME'],
                        ...
                        $rsSku = $el->GetList([], ["IBLOCK_ID" => $skuIblockId, "XML_ID" => $xmlId, "!PROPERTY_NOT_UPDATE_XML_FILE_VALUE" => "Y"], false, false, ["ID","PROPERTY_NO_AUTOUPDATE_ENUM"]);
                        ...
                        if($arSku = $rsSku->Fetch()) // если ТП найдено по внешнему коду, обновляем его
                        {
                            ...
                            {
                                $mxResult = CCatalogSku::GetProductInfo($arSku["ID"]); // получаем ID товара для этого ТП
                        ...
                        else // если ТП не найдено по внешнему коду, ищем товар по названию
                        {
                            ...
                            {
                                $script_create_sku_res = CIBlockPropertyEnum::GetList(
                                    [], Array(
                                        "IBLOCK_ID"=>$skuIblockId, "CODE"=>"SCRIPT_CREATE", "VALUE" => "Y"
                                    )
                                );
                                if( $script_create_sku_ob = $script_create_sku_res->GetNext() )
                                {
                                    $SCRIPT_CREATE_SKU_ENUM_ID = $script_create_sku_ob['ID'];
                                }
                                else
                                {
                                ...
                                $arProp = [
                                    ...
                                    'SCRIPT_CREATE' => ["VALUE" => $SCRIPT_CREATE_SKU_ENUM_ID]
                                ];
                            ...
                            else // если товар не найден по названию, создаем товар и ТП
                            {
                                ...
                                $script_create_res = CIBlockPropertyEnum::GetList( [], Array("IBLOCK_ID"=>$productIblockId, "CODE"=>"SCRIPT_CREATE", "VALUE" => "Y"));
                                if( $script_create_ob = $script_create_res->GetNext() ) {
                                    $SCRIPT_CREATE_ENUM_ID = $script_create_ob['ID'];
                                }
                                else
                                {
                                    // TODO Создать если нет
