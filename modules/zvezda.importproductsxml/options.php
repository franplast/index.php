...
<form method="post" action="<?=sprintf('%s?mid=%s&lang=%s&mid_menu=%s', $request->getRequestedPage(), urlencode($mid), LANGUAGE_ID)?>">
    ...
    ?>
        ...
        <tr>
            <td style="text-align: right"> <b>ИЛИ</b> </td>
        </tr>
        ...
        <tr>
            ...
            <td  colspan="2">
                <fieldset>
                    <legend><b>Опции для обновления товаров и SKU</b></legend>

                    <label style="width: 40%; display: inline-block; text-align: right;">Обновлять цену:</label>
                    <input type="checkbox" name="update_price" <?if(Option::get(MODULE_NAME, "update_price")):?>checked<?endif;?>><br><br>
                    <label style="width: 40%; display: inline-block; text-align: right;">Обновлять поля и свойства (кроме картинок):</label>
                    <input type="checkbox" name="update_properties" <?if(Option::get(MODULE_NAME, "update_properties")):?>checked<?endif;?>><br><br>
                    <label style="width: 40%; display: inline-block; text-align: right;">Обновлять картинки:</label>
                    <input type="checkbox" name="update_pictures" <?if(Option::get(MODULE_NAME, "update_pictures")):?>checked<?endif;?>><br><br>
                </fieldset>
    ...
    <?include $_SERVER["DOCUMENT_ROOT"]."/local/modules/zvezda.importproductsxml/tools/tab_instruction.php";?>
...
<script>
    ...
    $("#params").on('change', "input[name='ignore']", function()
    {
        ...
        $.ajax({
            type: "POST",
            url: "/local/modules/zvezda.importproductsxml/tools/ajax_ignore_param.php",
    ...
    $("#params").on('change', 'select', function()
    {
        ...
        $.ajax({
            type: "POST",
            url: "/local/modules/zvezda.importproductsxml/tools/ajax_select_prop.php",
    ...
    $("#params").on('click', 'a', function(e)
    {
        ...
        $.ajax({
            type: "POST",
            url: "/local/modules/zvezda.importproductsxml/tools/ajax_add_property.php",
    ...
    function requestScript(count = 10, num = 0)
    {
        ...
        $.ajax({
            type: "POST",
            url: "/local/modules/zvezda.importproductsxml/tools/ajax.php",
            data: data,
            success: function(data)
            {
                console.log(data);
